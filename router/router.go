...
func addRoutesForSO(router *gin.Engine) {
	...
	quarantineGroup := router.Group("/api/v1/so")
	{
		...
		quarantineGroup.POST("/notifyQuarantines", controller.SOController.NotifyQuarantines)
		quarantineGroup.POST("/notifyQuarantine", controller.SOController.NotifyQuarantine)
	}
}
...
func addRoutesForQuarantine(router *gin.Engine) {
	...
	quarantineGroup := router.Group("/api/v1/quarantine")
	{
		...
		quarantineGroup.GET("/downloadPhoto", controller.QuarantineController.DownloadPhoto)
